...
import { emails, subscriptions } from "../schema/emails";
...
export async function subscribe(categoryId: string, address: string) {
	try {
		...
		if (!email) {
			email = (
				await db.insert(emails).values({ address: address }).returning()
			).at(0);
			if (!email) return;
		}
		await db
			.insert(subscriptions)
			.values({ categoryId: categoryId, emailId: email.id });
		...
		await db
			...
			.where(
				and(
					...
					eq(subscriptions.emailId, email.id),
				),
			);
