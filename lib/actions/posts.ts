...
export async function createPost(postProp: typeof posts.$inferInsert): Promise<Post|null> {
    try {
...
export async function updatePost(postProp: Post):Promise<Post> {
    try {
        const post = (await db.update(posts).set(postProp).where(eq(posts.id, postProp.id)).returning()).at(0);
