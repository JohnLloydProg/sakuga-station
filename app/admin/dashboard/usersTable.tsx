		...
		<div>
			...
			<div className="min-w-200 w-full flex flex-col">
				...
				{users.map((user, index) => {
					...
					const borderbottom = index === users.length - 1 ? "border-b" : "";
					...
					return (
						<button
							...
							className={`grid grid-cols-[2fr_2fr_1fr_1fr] text-center items-stretch ${rowBg} cursor-pointer hover:bg-secondary/80 transition-colors duration-150 group border-accent ${borderbottom} ${selectedUser === user ? "bg-secondary/80" : "bg-transparent"}`}
						>
							<div
								className={`flex items-center gap-5 py-5 text-left px-6 font-medium border-accent border-l group-hover:text-accent font-josefin transition-colors ${selectedUser === user ? "text-accent" : "text-foreground"}`}
							>
								<p className="h-fit">
							...
							<div
								className={`flex items-center justify-center py-5 border-l border-accent px-4 truncate`}
							>
								{user.email}
							</div>
